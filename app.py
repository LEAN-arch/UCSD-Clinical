...
#
# VERSION: Final, Corrected (IndexError, FutureWarnings, and Stats Fixed)
#
...
#
# To Run:
# 1. Save this code as 'mcc_cto_final_corrected.py'
# 2. Create 'requirements.txt' with the specified libraries.
# 3. Install dependencies: pip install -r requirements.txt
# 4. Run from your terminal: streamlit run mcc_cto_final_corrected.py
#
# ======================================================================================
...
def generate_master_data():
    ...
    portfolio_data = {
        'Trial_ID': trial_ids, 'Trial_Type': np.random.choice(['Investigator-Initiated (IIT)', 'Industry-Sponsored', 'Cooperative Group'], num_trials, p=[0.4, 0.4, 0.2]),
        'Phase': np.random.choice(['I', 'I/II', 'II', 'III'], num_trials, p=[0.2, 0.3, 0.4, 0.1]), 'Disease_Team': np.random.choice(['Leukemia', 'Lung', 'Breast', 'GI', 'GU', 'Melanoma'], num_trials),
        'Status': np.random.choice(['Enrolling', 'Follow-up', 'Closed to Accrual', 'Suspended'], num_trials, p=[0.6, 0.2, 0.15, 0.05]), 'Subjects_Enrolled': np.random.randint(5, 100, num_trials),
        'PI_Experience_Level': np.random.choice(['Expert', 'Intermediate', 'New'], num_trials, p=[0.3, 0.5, 0.2]), 'Is_First_In_Human': np.random.choice([True, False], num_trials, p=[0.1, 0.9]),
    ...
    findings_data = {
        'Finding_ID': [f'FIND-{i:04d}' for i in range(1, num_findings + 1)], 'Trial_ID': np.random.choice(portfolio_df['Trial_ID'], num_findings),
        'Category': np.random.choice(finding_categories, num_findings, p=[0.3, 0.2, 0.15, 0.15, 0.1, 0.1]), 'Risk_Level': np.random.choice(['Critical', 'Major', 'Minor'], num_findings, p=[0.05, 0.35, 0.6]),
...
def plot_spc_chart(df, date_col, category_col, value, title):
    ...
    # --- FIX: Changed 'M' to 'ME' to fix FutureWarning ---
    ...
    # --- FIX: Use statistically correct c-chart formula for counts, fixes RuntimeWarning ---
...
def generate_ppt_report(kpi_data, spc_fig, findings_table_df):
    ...
    # --- FIX: Use enumerate with itertuples for a correct, sequential index to fix IndexError ---
...
with tab1:
    ...
    with col1:
        ...
        with st.spinner("Generating 12-month forecast..."):
            st.plotly_chart(generate_prophet_forecast_plot(findings_df), use_container_width=True)
    with col2:
        ...
        # [Prediction logic remains the same as previous version]
with tab2:
...
with tab3:
    ...
    st.info("**Actionable Insight:** This chart visualizes the Resource Strain Index. Auditors in the top-right quadrant ('High Strain') are potential burnout risks.")
...
st.sidebar.info("Click to download a PowerPoint summary of the current QA program status.")
